    ...
    <>
      {loading ? (
        ...
        <div>
          {token ? (
            ...
            <button
              onClick={() => navigate("/login")} // Navigate to login page if no token
              className="absolute top-5 right-5 bg-green-400 text-white px-4 py-1 rounded transition hover:bg-green-600 flex items-center z-20"
            >
              Login
            </button>
          )}
      ...
      <header className="absolute top-0 left-0 w-full flex items-center justify-between px-6 py-4 z-10"> {/* Added z-index to avoid overlap */}
        <div className="flex items-center">
          ...
          />
          {/* Navigation Links */}
          <nav className="flex space-x-14  text-gray-400 text-base">
            <a href="#" className="hover:border-b-2 hover:scale-105 hover:border-blue-500 hover:text-gray-200">Home</a>
            <a href="#about" className="hover:border-b-2 hover:scale-105 hover:border-blue-500 hover:text-gray-200">Explore</a>
            <a href="#contact" className="hover:border-b-2 hover:scale-105 hover:border-blue-500 hover:text-gray-200">Contact us</a>
            <a href="#contact" className="hover:border-b-2 hover:scale-105 hover:border-blue-500 hover:text-gray-200">About us</a>
            <a href="#contact" className="hover:border-b-2 hover:scale-105 hover:border-blue-500 hover:text-gray-200">Become a host</a>
      ...
      <div
        ...
        style={{
          backgroundImage: `linear-gradient(rgba(0, 0, 0, 0.65), rgba(0, 0, 0, 0.7)), url(http://localhost:5000/uploads/phot3.jpg)`, // Black gradient overlay
        }}
      >
        {/* Paragraph above the heading */}
        <div>
          <p className="text-gray-400 font-semibold text-lg mb-2" style={{ fontFamily: `'Jacques Francois', serif`, letterSpacing: '0.35em' }}>There   is   luxury   in   Simplicity</p>
        </div>

        {/* Heading with custom font */}
        <h1
          className="text-6xl font-semibold text-gray-300 bg-opacity-50 p-5"
        ...
        <button
          onClick={()=>{
            if (token) {
            navigate('/homstay-listing');
            } else {
              navigate('/login')
            }
          }} // Wrap navigate in an arrow function
